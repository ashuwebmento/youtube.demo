...
class Controller_Account extends Controller_Template_Generic {
	...
	public function action_create()
	{
		...
		if (isset($_POST) && Valid::not_empty($_POST)) {
			...
			->rule('postcode', Kohana::$config->load('clinic.account.create.postcode.formate'))
			->rule('postcode', 'min_length', array(':value', Kohana::$config->load('clinic.account.create.username.min_length')))
			->rule('postcode', 'max_length', array(':value', Kohana::$config->load('clinic.account.create.username.max_length')))
			
			->rule('city', 'not_empty')
			->rule('phone', 'not_empty')
			->rule('email', 'not_empty')
			->rule('email', 'email')
			->rule('re_email', 'not_empty')
			->rule('re_email', 'email')
			->rule('profession', 'not_empty')
			//->rule('photo', 'not_empty')
			->rule('tags', 'not_empty')
			->rule('description', 'not_empty')

			->rule('password', 'not_empty')
			->rule('password', 'min_length', array(':value', Kohana::$config->load('clinic.account.create.password.min_length')))
			...
			if ($post->check()) 
			{
				...
				$model->values(array(
					'fullname' 	=> HTML::entities(strip_tags($post['fullname'])),
					'email'		=> $post['email'],
					'password'	=> $post['password'],
					'address' 	=> HTML::entities(strip_tags($post['address'])),
					'postcode' 	=> HTML::entities(strip_tags($post['postcode'])),
					'city'	 	=> HTML::entities(strip_tags($post['city'])),
					'phone' 	=> HTML::entities(strip_tags($post['phone'])),
					'profession'=> HTML::entities(strip_tags($post['profession'])),
					'photo' 	=> $filename,
					'tags' 		=> HTML::entities(strip_tags($post['tags'])),
					'description'=> HTML::entities(strip_tags($post['description'])),
					'status' 	=> 1,
					'type' 		=> 'clinic',
				));
				...
				try {
					if($model->save())
					{
						$services = ORM::factory('user_services');
						$services->values(array(
							'user_id' 	=> $model->user_id,
							'service_name'		=> HTML::entities(strip_tags($post['service_name'])),
							'service_price' 	=> HTML::entities(strip_tags($post['service_price'])),
							'service_duration'	=> HTML::entities(strip_tags($post['service_duration'])),
						));
						$services->save();
						
						$organizations = ORM::factory('user_organizations');
						$organizations->values(array(
							'user_id' 	=> $model->user_id,
							'org_id'	=> $post['org_id'],
							'ondate' 	=> strtotime(now()),
						));
						$organizations->save();
					}
		...
		// TODO i18n
		$this->template->title = __('::BOOK &amp; CARE::Sign up');
